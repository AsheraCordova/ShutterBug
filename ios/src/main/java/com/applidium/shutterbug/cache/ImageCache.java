...
import r.android.app.ActivityManager;
...
public class ImageCache {
    ...
    private void openDiskCache() {
        File directory = null;
        if (r.android.os.Environment.getExternalStorageState().equals(r.android.os.Environment.MEDIA_MOUNTED)) {
            directory = new File(mContext.getExternalFilesDir(null), "Applidium Image Cache");
        } else {
        ...
        } catch (r.android.content.pm.PackageManager.NameNotFoundException e) {
            versionCode = 0;
            e.printStackTrace();
        }
        try {
            mDiskCache = DiskLruCache.open(directory, versionCode, DISK_CACHE_VALUE_COUNT, DISK_CACHE_MAX_SIZE);
        	 ...
        	 try {
				mDiskCache = DiskLruCache.open(mContext.getCacheDir(), versionCode, DISK_CACHE_VALUE_COUNT, DISK_CACHE_MAX_SIZE);
